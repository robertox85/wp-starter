...
class Widget extends WP_Widget {
	...
	var $slug = '';
	var $post_id = 0;
	var $data_id = 'starter_widget_'; // keep old name to preserve data
	var $classes = '';
	...
	function __construct( $params ) {
		...
		parent::__construct(
			$this->data_id,
			__( $this->title, 'starter' ), // Name
			array(
				'description' => __( $this->description, 'starter' ),
	...
	function form( $instance ) {
		...
		<?php endif; ?>
			...
			<p class='starter-no-acf'><?php _e( 'You have not added any fields to this widget yet.', 'starter' ); ?>

				<br/><br/>

				<a href='post-new.php?post_type=acf-field-group'><?php _e( 'Add some now!', 'starter' ); ?></a>

				<br/><br/>

				<?php _e( 'Make sure to set the location rules to:', 'starter' ); ?>
				
				<b>
				<?php
				_e( 'Widget : is equal to : ', 'starter' );
				echo $this->title;
				?>
				</b>
			...
			<script type='text/javascript'> starter();</script>
	...
	function widget( $args, $instance ) {
		...
		$custom_template_dir = trailingslashit( apply_filters( 'starter_custom_template_dir', get_template_directory_uri() ) );
	...
	public function display_titles() {
		...
		if ( apply_filters( "show_starter_title_{$this->slug}", false ) ) {
			return true;
		} elseif ( apply_filters( "hide_starter_title_{$this->slug}", false ) ) {
			return false;
		} elseif ( apply_filters( "show_starter_title_{$this->post_id}", false ) ) {
			return true;
		} elseif ( apply_filters( "hide_starter_title_{$this->post_id}", false ) ) {
			return false;
		} elseif ( apply_filters( 'show_starter_titles', false ) ) {
			return true;
		} elseif ( apply_filters( 'hide_starter_titles', false ) ) {
			return false;
		} else {
			return false;
		}
